...
public class RootNode extends SyntaxNode {
	...
	public RootNode(Location location, List<SyntaxNode> children) {
		super(location);
		this.children = children;
	}

	public static RootNode parse(TokenStream stream) {
		return new RootNode(stream.optionalLocation().orElse(Location.UNKNOWN), ComponentDefinitionNode.parseBody(stream, ComponentDefinitionNode::parseNode));
	}
