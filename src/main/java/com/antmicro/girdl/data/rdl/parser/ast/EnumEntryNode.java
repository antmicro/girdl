...
public class EnumEntryNode extends SyntaxNode {

	public final String name;
	public final ExpressionNode value;
	public final List<PropertyAssignmentNode> properties;

	public EnumEntryNode(Location location, String name, ExpressionNode value, List<PropertyAssignmentNode> properties) {
		super(location);
		this.name = name;
		this.value = value;
	...
	public static EnumEntryNode parse(TokenStream stream) {
		...
		if (stream.match("{")) {
			TokenStream inner = stream.block("{}", "enum properties");

			while (inner.hasNext()) {
				properties.add(PropertyAssignmentNode.parseExplicit(inner));
				inner.expect(";");
			}
		}

		return new EnumEntryNode(identifier, name, value, properties);
